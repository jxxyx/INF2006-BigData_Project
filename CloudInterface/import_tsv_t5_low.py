...
from myapp.models import Task5_low, UserProfile
...
# Reset the ID sequence for Task5_low model
reset_sequence(Task5_low)
...
for _, row in data.iterrows():
    ...
    Task5_low.objects.create(
        text=row['text'],
        airline_sentiment_gold=row['airline_sentiment_gold'],
        predicted_sentiment=row['predicted_sentiment_str'],
        correct=row['correct'],
        user=user
    )
