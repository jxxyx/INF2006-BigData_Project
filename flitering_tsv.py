...
# Preprocess data, apply sentiment analysis, and save results


def preprocess_and_save_data(input_file, output_file, stop_words):
    df = pd.read_csv(input_file, sep='\t')  # Read TSV file
    filtered_df = df[df['airline_sentiment_gold'].notna() & (
        df['airline_sentiment_gold'] != '')]
    filtered_df['predicted_sentiment'] = filtered_df['text'].apply(
        lambda text: get_sentiment_sentiwordnet(text, stop_words))
    filtered_df.to_csv(output_file, sep='\t', index=False)  # Save as TSV file
...
def load_and_analyze_data_spark(processed_file):
    ...
    # Load processed data
    df_spark = spark.read.csv(processed_file, sep='\t',
                              header=True, inferSchema=True)  # Load TSV file
    df_spark = df_spark.filter(df_spark.airline_sentiment_gold.isNotNull(
    ) & df_spark.predicted_sentiment.isNotNull())

    # Convert to Pandas for classification metrics
    pandas_df = df_spark.select(
        "airline_sentiment_gold", "predicted_sentiment").toPandas()

    # Calculate accuracy
    correct_predictions = (
        pandas_df['predicted_sentiment'] == pandas_df['airline_sentiment_gold']).sum()
    ...
    print(confusion_matrix(
        pandas_df['airline_sentiment_gold'], pandas_df['predicted_sentiment']))
